...
function createField() {
  ...
  for (let i = 0; i < 8; i++) {
    for (let j = 0; j < 8; j++) {
      ...
      if (i % 2 == 0 && j % 2 == 0) div.style.backgroundColor = colorTileLight;
      else if (i % 2 != 0 && j % 2 != 0)
        div.style.backgroundColor = colorTileLight;
      else div.style.backgroundColor = colorTileDark;
...
socket.on("update", ({ sender, backEndField }) => {
  ...
  if (
    ...
    if (
      destination == possibleRochadeRookL ||
      destination == possibleRochadeRookR
    ) {
      ...
      turn == "w"
        ? (destination.className = "white")
        : (destination.className = "black");
      origin.innerHTML = "";
      field[origin.id[0]][origin.id[2]] = 0;

      if (
        (turn == "w" ? destination.id[0] == 0 : destination.id[0] == 7) &&
        field[destination.id[0]][destination.id[2]].toLowerCase() == "p"
      ) {
        colorPromote = turn;
  ...
  //New Target
    ...
    if (
      field[row][column] != 0 &&
      (whiteTurnPiece(row, column) || blackTurnPiece(row, column))
    ) {
      //Upper Case letters are white pieces
      document.getElementById(sender.target.id).style.backgroundColor =
        colorBlue;
      markedId = pos;
...
function checkEnpassant(destination, sender) {
  if (destination.id == possibleEnPassant.id) {
    ...
    hitEnPassant != "9_9" &&
    destination.id == hitEnPassant.id &&
    field[sender.id[0]][sender.id[2]].toUpperCase() == "P"
  ) {
    enPassant.innerHTML = "";
...
function checkCheck() {
  ...
  if (wKingCheck) {
    ...
    document.getElementById(wKingPosition).style.backgroundColor =
      "transparent";
  }
  if (bKingCheck) {
    document.getElementById(bKingPosition).style.backgroundColor = "#FC0";
  } else {
    document.getElementById(bKingPosition).style.backgroundColor =
      "transparent";
  }
}
...
function colorPossibleMovement() {
  possibleMovement.forEach((tile) => {
    if (field[tile.id[0]][tile.id[2]] == 0)
      ...
      blackTurnPiece(tile.id[0], tile.id[2]) ||
      whiteTurnPiece(tile.id[0], tile.id[2])
    )
      tile.style.backgroundColor = colorBlue;
    else tile.style.backgroundColor = colorRed;
  });
}
...
function updateRochade(row, column) {
  if (turn == "w") {
    if (row == 7 && column == 0) rookLeftMovedW = true;
    if (row == 7 && column == 7) rookRightMovedW = true;
  } else {
    if (row == 0 && column == 0) rookLeftMovedB = true;
    if (row == 0 && column == 7) rookRightMovedB = true;
  }
}
...
function switchPiece(sender) {
  field[promotedPawn.id[0]][promotedPawn.id[2]] =
    colorPromote == "w" ? sender.id.toUpperCase() : sender.id.toLowerCase();
...
function isThreatened(row, column, color) {
  ...
  color == "w"
    ? (field[wKingPosition[0]][wKingPosition[2]] = 0)
    : (field[bKingPosition[0]][bKingPosition[2]] = 0);
  ...
  field.forEach((fieldrow) => {
    for (let i = 0; i < 8 && !threatened; i++) {
      ...
      if (
        possibleMovement.indexOf(document.getElementById(row + "_" + column)) !=
        -1
      ) {
        threatened = true;
      }
...
function afterMovementOwnKingInCheck(
  destinationRow,
  destinationColumn,
  originRow,
  originColumn,
  ...
  let possible =
    color == "w"
      ? isThreatened(wKingPosition[0], wKingPosition[2], "w")
      : isThreatened(bKingPosition[0], bKingPosition[2], "b");
...
function checkifMovementPossible(rowFrom, columnFrom) {
  if (field[rowFrom][columnFrom].toLocaleLowerCase() == "k") return;
  let tmpCheckPossibleMovement = [];
  possibleMovement.forEach((target) => {
    if (
      !afterMovementOwnKingInCheck(
        target.id[0],
        target.id[2],
        rowFrom,
        columnFrom,
        isWhite(rowFrom, columnFrom) ? "w" : "b"
      )
    )
...
function resolveRochade(direction) {
  let row = turn == "w" ? "7" : "0";
  let rookFrom = document.getElementById(
    direction == "l" ? row + "_0" : row + "_7"
  );
  let rookTo = document.getElementById(
    direction == "l" ? row + "_3" : row + "_5"
  );
  let kingFrom = document.getElementById(row + "_4");
  let kingTo = document.getElementById(
    direction == "l" ? row + "_2" : row + "_6"
  );
...
function ColorRed(rowTo, columnTo, color, rowFrom, columnFrom) {
  if (inBound(rowTo, columnTo)) {
    if (field[rowTo][columnTo] == 0) return false;
...
function ColorRedKing(rowTo, columnTo, color, rowFrom, columnFrom) {
  if (inBound(rowTo, columnTo)) {
    if (field[rowTo][columnTo] == 0) return false;
    if (
      takeable(rowTo, columnTo, color) &&
      !isThreatened(rowTo, columnTo, color)
    ) {
      let tile = document.getElementById(rowTo + "_" + columnTo);
      possibleMovement.push(tile);
      return true;
...
function ColorRedEnPassant(rowTo, columnTo, rowFrom, columnFrom) {
  if (inBound(rowTo, columnTo)) {
    if (enPassant != "9_9") {
      if (
        document.getElementById(rowTo + "_" + columnTo).id != hitEnPassant.id
      ) {
        return false;
      }
      let tile = document.getElementById(rowTo + "_" + columnTo);
